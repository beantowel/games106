...
size_t getAlignment(vks::VulkanDevice* device, size_t size) {
...
public:
	...
	*/
	...
	struct NodeDataDynamic {
		...
		Values* values; // aligned array

		auto getPointer(uint32_t index)
		{
			auto rawPtr = reinterpret_cast<uint64_t>(values) + (index * dynamicAlignment);
			auto ptr = reinterpret_cast<decltype(values)>(rawPtr);
			return ptr;
		}
	};
	...
	*/
	...
	NodeDataDynamic nodeData;
	uint32_t activeAnimation = 0;

	~VulkanglTFModel()
	{
		if (nodeData.values) {
			alignedFree(nodeData.values);
		}
		for (auto node : nodes) {
			delete node;
		}
	...
	*/
	...
	void loadMaterials(tinygltf::Model& input)
	{
		...
		for (size_t i = 0; i < input.materials.size(); i++) {
			...
			auto haskey = [&](const std::string name) {
				return glTFMaterial.values.find(name) != glTFMaterial.values.end();
			};
			// Get the base color factor
			if (haskey("baseColorFactor")) {
				materials[i].values.baseColorFactor = glm::make_vec4(glTFMaterial.values["baseColorFactor"].ColorFactor().data());
			}
			if (haskey("metallicFactor")) {
				materials[i].values.metallicFactor = static_cast<float>(glTFMaterial.values["metallicFactor"].Factor());
			}
			if (haskey("roughnessFactor")) {
				materials[i].values.roughnessFactor = static_cast<float>(glTFMaterial.values["roughnessFactor"].Factor());
			}
			if (haskey("baseColorTexture")) {
				materials[i].baseColorTextureIndex = glTFMaterial.values["baseColorTexture"].TextureIndex();
			}
			if (haskey("metallicRoughnessTexture")) {
				materials[i].metallicRoughnessTextureIndex = glTFMaterial.values["metallicRoughnessTexture"].TextureIndex();
			}
			// not in values
			materials[i].values.emissiveFactor = glm::make_vec4(glTFMaterial.emissiveFactor.data());
			materials[i].normalTextureIndex = glTFMaterial.normalTexture.index;
			materials[i].emissiveTextureIndex = std::max(0, glTFMaterial.emissiveTexture.index);
		}
	}

	void updateNode(Node* node)
	{
		const auto ptr = nodeData.getPointer(node->index);
		ptr->model = node->getMatrix();
		for(auto &child: node->children) {
			updateNode(child);
		}
	}

	void loadNode(uint32_t nodeIndex, const tinygltf::Model& input, VulkanglTFModel::Node* parent, std::vector<uint32_t>& indexBuffer, std::vector<VulkanglTFModel::Vertex>& vertexBuffer)
	{
		VulkanglTFModel::Node* node = new VulkanglTFModel::Node{};
		node->matrix = glm::mat4(1.0f);
		node->parent = parent;
		node->index = nodeIndex;
		const auto &inputNode = input.nodes[nodeIndex];
		...
		if (inputNode.children.size() > 0) {
			for (size_t i = 0; i < inputNode.children.size(); i++) {
				loadNode(inputNode.children[i], input , node, indexBuffer, vertexBuffer);
			}
		}
...
size_t VulkanglTFModel::Node::countNodes()
{
	size_t cnt = 1;
	for(auto &child: children) {
		cnt += child->countNodes();
	}
	return cnt;
}
...
public:
	...
	struct DescriptorSetLayouts {
		...
		VkDescriptorSetLayout matricesDynamic;
	...
	~VulkanExample()
	{
		...
		vkDestroyDescriptorSetLayout(device, descriptorSetLayouts.matricesDynamic, nullptr);
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			// Bind scene matrices descriptor to set 0 (for object)
			vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &shaderData.descriptorSet, 0, nullptr);
			// Objects
	...
	void loadglTFFile(std::string filename, VulkanglTFModel& glTFModel)
	{
		...
		if (fileLoaded) {
			...
			for (size_t i = 0; i < scene.nodes.size(); i++) {
				glTFModel.loadNode(scene.nodes[i], glTFInput, nullptr, indexBuffer, vertexBuffer);
			}
		...
		// Uniform buffer object with per-node matrices
		auto &ndata = glTFModel.nodeData;
		ndata.dynamicAlignment = getAlignment(vulkanDevice, sizeof(decltype(ndata.valueTypeStruct)));
		ndata.bufferSize = 0;
		for (auto &node: glTFModel.nodes) {
			ndata.bufferSize += node->countNodes() * ndata.dynamicAlignment;
		}
		ndata.values = static_cast<decltype(ndata.values)>(alignedAlloc(ndata.bufferSize, ndata.dynamicAlignment));
		assert(ndata.values);
		std::cout << "dAlign=" << ndata.dynamicAlignment << std::endl;
		std::cout << "bufferS=" << ndata.bufferSize << std::endl;
		VK_CHECK_RESULT(vulkanDevice->createBuffer(
		...
		ndata.buffer.descriptor.range = ndata.dynamicAlignment;
		VK_CHECK_RESULT(ndata.buffer.map());

		// material buffer
		auto &materials = glTFModel.materials;
		for(auto& mat: materials) {
			...
			&mat.buffer,
			sizeof(mat.values)));
			VK_CHECK_RESULT(mat.buffer.map());
			memcpy(mat.buffer.mapped, &mat.values, sizeof(mat.values));
			mat.buffer.flush();
			mat.buffer.unmap();
		}
	}
	...
	void setupDescriptors()
	{
		...
		*/
		...
		const uint32_t maxSetCount = 100;
		VkDescriptorPoolCreateInfo descriptorPoolInfo = vks::initializers::descriptorPoolCreateInfo(poolSizes, maxSetCount);
		VK_CHECK_RESULT(vkCreateDescriptorPool(device, &descriptorPoolInfo, nullptr, &descriptorPool));

		VkDescriptorSetLayoutBinding setLayoutBinding;
		VkDescriptorSetLayoutCreateInfo descriptorSetLayoutCI = vks::initializers::descriptorSetLayoutCreateInfo(&setLayoutBinding, 1);
		// Descriptor set layout for passing matrices, binding=0
		...
		// Descriptor set layout for passing dynamic matrices, set=2, binding=0
		setLayoutBinding = vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0);
		VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device, &descriptorSetLayoutCI, nullptr, &descriptorSetLayouts.matricesDynamic));

		// Descriptor set layout for passing material textures, set=1, binding=0..5
		std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
		// color, normal, metallicRoughness, emissive, brdfLUT, prefilter
			vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 0),
		...
		// set 0 = matrices (VS)
		// set 1 = material (FS)
		// set 2 = matrices dynamic (VS)
		std::array<VkDescriptorSetLayout, 3> setLayouts = {
			descriptorSetLayouts.matrices,
			descriptorSetLayouts.materials,
			descriptorSetLayouts.matricesDynamic,
		};
		...
		VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &shaderData.descriptorSet));
		VkWriteDescriptorSet writeDescriptorSet = vks::initializers::writeDescriptorSet(shaderData.descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &shaderData.buffer.descriptor);
		vkUpdateDescriptorSets(device, 1, &writeDescriptorSet, 0, nullptr);


		// Descriptor set for instance matrices
		auto &ndata = glTFModel.nodeData;
		allocInfo = vks::initializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayouts.matricesDynamic, 1);
		VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &ndata.descriptorSet));
		writeDescriptorSet = vks::initializers::writeDescriptorSet(ndata.descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, 0, &ndata.buffer.descriptor);
		vkUpdateDescriptorSets(device, 1, &writeDescriptorSet, 0, nullptr);
		...
		for(auto& mat: glTFModel.materials){
			...
			auto writeSet = [&](uint32_t texIndex, uint32_t binding){
				auto imgIndex = glTFModel.textures[texIndex].imageIndex;
				auto* pDescriptor = &glTFModel.images[imgIndex].texture.descriptor;
				return vks::initializers::writeDescriptorSet(mat.descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, binding, pDescriptor);
			};
			std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
				writeSet(mat.baseColorTextureIndex, 0),
				writeSet(mat.normalTextureIndex, 1),
				writeSet(mat.metallicRoughnessTextureIndex, 2),
				writeSet(mat.emissiveTextureIndex, 3),
				vks::initializers::writeDescriptorSet(mat.descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 4, &textures.lutBrdf.descriptor),
				vks::initializers::writeDescriptorSet(mat.descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 5, &textures.prefilteredCube.descriptor),
				vks::initializers::writeDescriptorSet(mat.descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 6, &mat.buffer.descriptor),
			};
	...
	void prepareUniformBuffers()
	{
		...
		updateDynamicUniformBuffer(glTFModel);
	}
	...
	void updateDynamicUniformBuffer(VulkanglTFModel& glTFModel)
	{
		auto &ndata = glTFModel.nodeData;
		memcpy(ndata.buffer.mapped, ndata.values, ndata.bufferSize);
		// Flush to make changes visible to the host
		VkMappedMemoryRange memoryRange = vks::initializers::mappedMemoryRange();
		memoryRange.memory = ndata.buffer.memory;
		memoryRange.size = ndata.buffer.size;
